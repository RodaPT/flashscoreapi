        ...
        for url in urls:
            ...
            for panel in match_panels:
                ...
                for match in matches:
                    ...
                    # Send a GET request to the match page
                    match_response = requests.get(match_link, headers=headers)
                    match_response.raise_for_status()  # Raise an exception for bad status codes

                    # Parse the HTML content of the match page
                    match_soup = BeautifulSoup(match_response.content, 'html.parser')
                    
                    stadium_name_tag = match_soup.find('a', attrs={'data-cy': 'stadium'}).find('b')
                    if stadium_name_tag:
                        stadium_name = stadium_name_tag.get_text(strip=True)
                    else:
                        raise ValueError("Stadium name tag not found")
